...
public class RoundGroupBullet : MonoBehaviour
{
    ...
    IEnumerator FirRoundGroup()
        ...
        Quaternion rotateQuate = Quaternion.AngleAxis(90, Vector3.forward);//使用四元数制造绕Z轴旋转45度的旋转
        while (CountTime < StopTime)
        {
            ...
            for (int i = 0; i < bullets.Count; i++)
            {
                ...
                StartCoroutine(FirRound(4, bullets[i].transform.position));//通过之前弹幕的位置，生成多波多方向的圆形弹幕
            }
        }
